...
<h1 align="center"> Folios de: {{$cliente->nombre_empresa}}</h1>
    ...
    <div class="row">
        <div class="col-3" id="caja1">
            <div class="container">
                <form action="/FolioVentas" method="POST">
                    @csrf
        ...
        <div class="col-6" id="caja2">    
            <div class="container" >
                <table class="table" >
                    ...
                    <tbody>
                        @foreach ($FolioVenta as $item)
                            <tr>
                                {{-- <td>{{$item->id}}</td> --}}
                                <td><a href="/ventas/{{$item->id}}" class="btn btn-primary">Agregar Facturas</a></td>
                                <td>{{$item->anio}}</td>
                                <td>{{$item->mes}}</td>
                               ...
                               {{--  <td>{{$item->id_cliente}}</td> --}}
                                <td>
                                    ...
                                    <form action="/FolioVentas/{{$item->id}}" method="POST" class="d-inline">
                                        @method('DELETE')
                                        @csrf
